...
class SignedPreKeyEntity {
  ...
  /**
   * Builds a persistable signed prekey row.
   *
   * @param deviceId the owning device UUID
   * @param keyId the libsignal key id
   * @param publicKey 33-byte Curve25519 public key
   * @param signature 64-byte key signature
   */
  SignedPreKeyEntity(UUID deviceId, int keyId, byte[] publicKey, byte[] signature) {
  ...
  /** Composite key class for {@link SignedPreKeyEntity}. */
